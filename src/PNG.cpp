...
PNG::PNG(std::string filename) {
    std::vector<unsigned char> data = Utils::read_file(filename, std::fstream::binary);
    this->init(data);
}
...
void PNG::init(std::vector<unsigned char> &data) {
    ...
    if(this->is_supported_color_type(ihdr->type) == false) {
        throw GenericException("Unsupported color type", Code::UNSUPPORTED_COLOR_TYPE);
    }
    if(ihdr->type == IHDR::Color_type::RGBA || ihdr->type == IHDR::Color_type::GREYSCALE_APLHA) {
    ...
    } else if(ihdr->type == IHDR::Color_type::GREYSCALE || ihdr->type == IHDR::Color_type::GREYSCALE_APLHA) {
...
std::vector<unsigned char> PNG::remap_palette(std::vector<unsigned char> &indexes) {
...
std::vector<unsigned char> PNG::grayscale_to_rgb(std::vector<unsigned char> &grayscale) {
...
void PNG::decode_line(unsigned char *destination, unsigned char *source, int line_number) const {
    Vector2<int> image_size = this->get_size();
...
void PNG::load_chunks(std::vector<unsigned char> &buffer) {
    ...
    while(ptr < &buffer.back()) {
        ...
        ptr += (sizeof(Chunk::length) + sizeof(Chunk::type) + sizeof(Chunk::crc) + length);
    }

    if(this->are_critical_chunks_present() == false) {
        throw GenericException("Critical chunks not found", Code::CRITICAL_CHUNKS_NOT_FOUND);
    }
}