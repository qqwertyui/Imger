...
Utils::Chunk_manager::~Chunk_manager() {
  for (Utils::Data_vector::iterator i = this->entry.begin(); i != this->entry.end();
       i++) {
    delete[](*i)->data;
  }
  for (Utils::Data_vector::iterator i = this->entry.begin(); i != this->entry.end();
...
std::vector<unsigned char> Utils::zlib_inflate(std::vector<unsigned char> &input) {
    ...
    memcpy(result.data(), container.to_bytearray(), result.capacity());
    return result;
}

uint32_t Utils::crc32(unsigned char *data, size_t datasz) {
...
uint32_t Utils::se32(uint32_t number) {
	return __builtin_bswap32(number);
}

uint16_t Utils::se16(uint16_t number) {
...
std::vector<unsigned char> Utils::read_file(std::string &filename, std::ifstream::openmode flags) {
