...
ICO::ICO(std::vector<unsigned char> &data) {
    this->init(data);
}
...
Vector2<int> ICO::get_size() const {
    return this->image->get_size();
}
...
unsigned char* ICO::get_pixels() const {
    return this->image->get_pixels();
}
