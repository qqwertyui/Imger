...
BMP::BMP(std::vector<unsigned char> &data) { this->init(data); }
...
void BMP::init(std::vector<unsigned char> &data) {
  ...
  if (this->dib.bpp != 24 && this->dib.bpp != 32) {
    throw GenericException("Unsupported color depth",
                           Code::UNSUPPORTED_COLOR_DEPTH);
  }

  if (this->dib.compression_method != 0) {
    throw GenericException("Unsupported compression method",
...
unsigned int BMP::get_bytes_per_pixel() const { return (this->dib.bpp / 8); }

bool BMP::DIB::is_valid(unsigned char *bytes) {
  if (memcmp(bytes, BMP::DIB::SIGNATURE.data(), BMP::DIB::SIGNATURE.size()) !=
      0) {
    return false;
  }
  return true;
