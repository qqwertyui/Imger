...
SDLW::SDLW(std::string window_title, unsigned int window_size_percent, Image *image)
    : window_title(window_title)
{
	...
	this->window = SDL_CreateWindow(this->get_formatted_window_title().c_str(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, this->window_size.x, this->window_size.y, SDL_WINDOW_RESIZABLE);
...
SDL_PixelFormatEnum SDLW::bytes_per_pixel_to_sdl_format(unsigned int bytes_per_pixel) {
...
void SDLW::set_active_image(Image *image) {
    ...
    SDL_PixelFormatEnum pixel_format = SDLW::bytes_per_pixel_to_sdl_format(image->get_bytes_per_pixel());
    this->texture = SDL_CreateTexture(this->renderer, pixel_format, SDL_TEXTUREACCESS_STATIC, image_size.x,image_size.y);
	...
	this->rect.x = (double)this->window_size.x/2.f - this->image_size.x*this->zoom/2.f;
	this->rect.y = (double)this->window_size.y/2.f - this->image_size.y*this->zoom/2.f;
...
void SDLW::draw() {
	...
	SDL_SetRenderDrawColor(this->renderer, this->background_color.r, this->background_color.g, this->background_color.b, 255);
...
void SDLW::scroll_zoom(SDL_Event *e) {
    ...
    Vector2<int> diff(this->rect.w-this->image_size.x*this->zoom, this->rect.h-this->image_size.y*this->zoom);
...
bool SDLW::fits_bounds(int xdir, int ydir) {
    if(this->rect.x < -(this->image_size.x*this->zoom) && xdir < 0) {
        return false;
    } if(this->rect.x > (this->window_size.x+this->image_size.x*this->zoom) && xdir > 0) {
        return false;
    } if(this->rect.y < -(this->image_size.y*this->zoom) && ydir < 0) {
        return false;
    } if(this->rect.y > (this->window_size.y+this->image_size.y*this->zoom) && ydir > 0) {
        return false;
    }
    return true;
}
...
void SDLW::set_margin(unsigned int length) {
    this->margin_length = length;
}

std::string SDLW::get_formatted_window_title() const {
    std::ostringstream out;
    out<<this->window_title<<" | "<<this->image_size.x<<" x "<<this->image_size.y<<" | "<<round(this->zoom*100)<<"%";
    return out.str();
}
...
void SDLW::update_position_on_resize(Vector2<int>&new_size) {
    Vector2<double> position_percent((double)this->rect.x/(double)this->window_size.x, (double)this->rect.y/(double)this->window_size.y);
